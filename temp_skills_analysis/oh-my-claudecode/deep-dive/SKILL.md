...
- 状态通过将状态写入 `.trae/state/deep-interview-state.json` 持久化，带 `source: "deep-dive"` 区分符
...
3. **检测棕地 vs 绿地**：
   - 运行 @search 智能体：检查当前目录是否有现有源代码、包文件或 git 历史
...
4. **生成 3 条 trace 通道假设**：
   - 默认通道（除非问题强烈暗示更好的分区）：
     ...
     2. **配置 / 环境 / 编排原因**
     3. **度量 / 制品 / 假设不匹配原因**
   - 对于棕地：运行 @search 智能体识别相关代码库区域，存储为 `codebase_context` 供后续注入
4.5. **加载运行时设置**：
...
保存后：
- 在状态中持久化 `trace_path`：将状态写入 `.trae/state/deep-interview-state.json` 设置 `state.trace_path = ".trae/specs/deep-dive-trace-{slug}.md"`
...
- 在状态中持久化 `spec_path`：将状态写入 `.trae/state/deep-interview-state.json` 设置 `state.spec_path = ".trae/specs/deep-dive-{slug}.md"`
...
- 使用 @search 智能体进行棕地代码库探索（Phase 1）
- 使用并行模式运行 3 条 tracer 通道（Phase 3）
- 使用将状态写入 `.trae/state/deep-interview-state.json` 带 `state.source = "deep-dive"` 进行所有状态持久化
- 使用读取 `.trae/state/deep-interview-state.json` 进行恢复——检查 `state.source === "deep-dive"` 以区分
...
如果中断，再次运行 deep-dive 技能。技能从读取 `.trae/state/deep-interview-state.json` 读取状态并检查 `state.source === "deep-dive"` 以从最后完成的阶段恢复。制品路径（`trace_path`、`spec_path`）从状态重建，而非对话历史。状态模式与 deep-interview 的期望兼容，因此 Phase 4 interview 机制无缝工作。
