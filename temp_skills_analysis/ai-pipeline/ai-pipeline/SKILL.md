...
**What happens**: Invoke the `external-research` skill (Skill tool: `external-research`), passing:
...
**What to do**:
1. Load the blueprint skill (Skill tool: `blueprint`)
...
**What to do**:
1. Load the architecture-decision-records skill (Skill tool: `architecture-decision-records`)
...
**What to do**:
1. Load the eval-harness skill (Skill tool: `eval-harness`)
...
**Output checkpoint**:
- Eval definitions in `.claude/evals/`
...
**What to do**:
1. Load the self-improve skill (Skill tool: `self-improve`)
...
- Progress chart at `.omc/self-improve/topics/{slug}/tracking/progress.png`
...
**What to do**:
1. Load the ai-slop-cleaner skill (Skill tool: `ai-slop-cleaner`)
...
- ADRs: docs/adr/
- Evals: .claude/evals/
- Progress chart: .omc/self-improve/topics/{slug}/tracking/progress.png
- Pipeline state: .omc/pipeline-state.json
...
Track pipeline progress in `.omc/pipeline-state.json`:
...
1. Read `.omc/pipeline-state.json`
