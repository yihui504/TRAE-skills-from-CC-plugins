...
3. **先收集代码库事实**：在问用户之前，先用 @search 智能体查找
4. **基于答案构建**：每个问题建立在前一个答案之上
5. **咨询分析师**：调用 @backend-architect 智能体发现隐藏需求、边缘情况和风险
6. 当用户表示准备好时创建计划
...
2. **Architect** 调用 @backend-architect 智能体审查架构合理性：
   - 必须提供最强反论（反题）
   - 至少一个有意义的权衡张力
   - 可能的话提供综合路径
   - **等待此步骤完成后再进入步骤3**

3. **Critic** 调用 @backend-architect 智能体评估质量标准：
   - 验证原则-选项一致性
   - 公平的替代方案探索
   - 风险缓解清晰度
   - 可测试的验收标准
...
2. 调用 @backend-architect 智能体（带审查指令）通过Critic评估
...
- 使用纯文本进行需要具体值的问题
- 使用 @search 智能体在问用户之前收集代码库事实
- **共识模式智能体调用必须顺序执行，不可并行**
- 需求足够清晰时停止访谈——不要过度访谈
