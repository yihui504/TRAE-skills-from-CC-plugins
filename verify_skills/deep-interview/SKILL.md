...
2. **检测棕地 vs 绿地**：
   ...
   - 否则：**绿地**
3. **对于棕地**：运行 @search 智能体映射相关代码库区域，存储为 `codebase_context`
3.5. **加载运行时设置**：
...
通过将状态写入对应的 `.trae/state/` JSON 文件用新轮次和分数更新 interview 状态。
...
**重要：** 选择执行方式时，**必须**通过技能调用传递规格路径。不要直接实现。Deep-interview 是需求技能，不是执行技能。如果过大的初始上下文已被摘要，向前传递规格和提示安全摘要，而非原始过大源材料。
...
- 使用 @search 智能体进行棕地代码库探索（在向用户提问之前运行）
- 使用高能力模型（温度 0.1）进行模糊度评分——一致性至关重要
- 使用文件系统读写 `.trae/state/` 下的 JSON 文件进行 interview 状态持久化
- 使用写入工具保存最终规格到 `.trae/specs/`
- 使用技能调用桥接到执行模式——绝不直接实现
- Challenge 代理模式是提示注入，不是单独的智能体生成
...
```
[生成 @search 智能体: "find authentication implementation"]
