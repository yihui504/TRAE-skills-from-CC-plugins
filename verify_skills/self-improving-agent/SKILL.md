...
> **注意：** hooks 目录中的脚本文件仍然保留，但在 TRAE IDE 中不会自动执行。需要手动触发或适配为 TRAE IDE 的事件机制。以下 Claude Code 的 hooks 配置方式在 TRAE IDE 中不支持。

### 在 TRAE IDE 中配置 Hooks

在原 Claude Code 中，hooks 通过 settings 配置（`~/.trae-cn/settings.json`）。TRAE IDE 不支持此配置方式，需要通过其他方式实现类似功能：
...
Replace `${SKILLS_DIR}` with your actual skills path (`~/.trae-cn/skills`).
